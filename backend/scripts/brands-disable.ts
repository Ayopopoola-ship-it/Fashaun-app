...
async function main(): Promise<void> {
  ...
  if (error) {
    throw new Error(`Failed to disable brand (${domain}): ${error.message}`);
  }
  ...
  logger.info(`Disabled brand: ${data[0].name} (${data[0].domain})`);
}

main().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : 'Unknown error';
  logger.error(`brands:disable failed: ${message}`);
  process.exit(1);
});
