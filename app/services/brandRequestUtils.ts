import { BrandRequestStatus, BrandSourceType, DifficultyTag, EstimatedOnboardingTime } from '../types';
...
export function difficultyLabel(difficulty: DifficultyTag): string {
  if (difficulty === 'easy') {
    return 'Easy to add';
  }
...
export function onboardingWindowLabel(value: EstimatedOnboardingTime): string {
  if (value === '1_to_3_days') {
    return '1 to 3 days';
  }

  if (value === '3_to_7_days') {
    return '3 to 7 days';
  }

  if (value === '1_to_3_weeks') {
    return '1 to 3 weeks';
  }
...
export function buildBrandRequestShareLink(requestId: string): string {
  return `fashaun://request/${requestId}`;
}
